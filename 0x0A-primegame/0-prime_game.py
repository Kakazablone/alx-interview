...
def sieve_of_eratosthenes(n):
    ...
    Returns:
    list: A list where the value at index i is True if i is a prime, False otherwise.
    """
...
def isWinner(num_rounds, nums):
    ...
    Maria and Ben take turns choosing a prime number and removing that number and its
    multiples from the set. The player unable to make a move loses the round.
    
    Args:
    num_rounds (int): The number of rounds to play.
    nums (list): A list of integers, where each integer represents the upper bound (n)
                 for that round.
    
    Returns:
    str: The name of the player who won the most rounds ("Maria" or "Ben"), or None
